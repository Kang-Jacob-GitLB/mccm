...
|---|---|---|---|---|---|
| System Setting 시간/언어 | WDSW2D2510-297 | SUMMARY ↔ Time/Language | 높음 | 2h | (4줄) … |

#### 5.3 dry-run → 멀티라인 코멘트
추정 매핑을 TSV(`KEY \t TIME \t STARTED \t COMMENT`)로 만들어 `jira_worklog.sh` 에 넘긴다. **코멘트 멀티라인은 `COMMENT` 안에 리터럴 `\n` 으로** 넣는다 — 스크립트가 apply 시 실제 줄바꿈으로 확장한다(손수 `jira` 호출 불필요).
```bash
SH="$SKILL_DIR/jira_worklog.sh"
printf '%s\n' \
  $'WDSW2D2510-297\t2h\t2026-05-29 18:30:00\t- Time Settings 미세조정\\n- 년 하한 2000\\n- Language 한글 기본\\n- 빌드/보드 반영' \
  | "$SH"            # dry-run (시간·시작시각 30분 반올림, 겹침회피, (N줄) 표기)
```
- **시간/시작시각 30분 그리드 nearest 반올림**(시간은 0<최소 30m; 09:53→10:00). `--round 0` 으로 끔.
...
- 코멘트 라인 수는 소요시간당 ~1줄(2h30m→3줄). dry-run 은 첫 줄 + `(N줄)` 로 미리보기.
...
- 종일·참석거절(`declined`) 제외. 회의를 `event:"meeting"` 라인으로 합쳐 타임라인 마커(`▣`)·활동시간 합산·겹침회피 대상(찬 구간)으로 사용.
...
워크로그 · {YYYY-MM-DD} ({요일})                       활동 {합계} · 커밋 {N}
 ...
 ▐ WDSW2D2510-297 ▌ ████████  18:30  System Setting 시간/언어 (4줄)  진행중  ✔
...
```
렌더 규칙:
- **24h 축**: 폭 W(44~48열). 막대 시작/끝 열 = `round(hour/24*W)`. hour 는 prep.sh 가 준 **로컬(KST) 시각**(⚠ UTC 로 그리지 말 것). 문자: 코딩 `▓`·회의 `█`·점심/회피 `▒`·빈칸 `·`. 축 눈금과 좌측 경계 `│` 세로 정렬.
- 각 막대 우측 `HH:MM–HH:MM · 길이` 가 활동시간 정보를 겸한다(별도 표 불필요).
- **DRY-RUN 행**: `▐ 전체KEY ▌`(숫자만 줄이지 말 것 — 오입력 방지) + 30m 채움막대(`█`=소요,`░`=잔여) + STARTED + 코멘트(첫줄 + `(N줄)`) + 꼬리표 `진행중 ✔`/`⚠완료·보류`/`이슈미정`/`∅`. `⚠완료`·`이슈미정`·`∅` 는 apply 제외. 마지막 줄에 등록예정/보류/확인 건수.
